...
async function removeOrphanGuestFeedItems (){
  ...
  cursor.forEach(item => {
    console.log('delete guestfeedietm', item._id)
    client.db(process.env.DB_DATABASE_NAME).collection('guestfeeditems').deleteOne({_id:item._id});
  })
