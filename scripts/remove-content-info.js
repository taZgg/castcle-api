...
async function removeOrphanFeedItems (){
  ...
  const cursor =await client.db(process.env.DB_DATABASE_NAME).collection('contentinfo').aggregate([
    {
      '$lookup': {
        'from': 'contents', 
  ...
  cursor.forEach(item => {
    ...
    client.db(process.env.DB_DATABASE_NAME).collection('contentinfo').deleteOne({_id:item._id});
  })
